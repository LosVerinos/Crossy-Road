...
public class TerrainGenerator : MonoBehaviour
{
    ...
    private int maxTerrainCount = 100;
        ...
        if (currentPosition.x - playerPos.x < minDistanceFromPlayer || isStart)
        {
            ...
            for (var i = 0; i < successive; i++)
            {
                ...
                if (terrainData[wichTerrain].name.StartsWith("Water"))
                {
                    ...
                    {
                        var southCliff = terrainData[wichTerrain].PossibleTerrain[whichOne].transform
                            .Find("cliff-South.vox");
                        southCliff.gameObject.SetActive(true);

                        if (lastTerrain.transform.name.StartsWith("Water") ||
                            lastTerrain.transform.name.StartsWith("Lilipads")) southCliff.gameObject.SetActive(false);
                    }
                }
                else
                {
                    ...
                    if (GlobalVariables.theme == "StarWars" && currentPosition.x > 6)
                    {
                        var northCliff = terrainData[wichTerrain].PossibleTerrain[whichOne].transform.Find("Cliff");
                        if (northCliff != null)
                        {
